...
echo "[RESTART] Working dir: $ROOT_DIR"
...
if ss -ltn | grep -q ":${REDIS_PORT}"; then
  echo "[RESTART] Redis already on :${REDIS_PORT}"
else
  if command -v redis-server >/dev/null 2>&1; then
    echo "[RESTART] Starting redis-server ..."
    redis-server --daemonize yes >/dev/null 2>&1 || true
    sleep 1
  else
    echo "[RESTART] Starting Redis via Apptainer ..."
...
if ss -ltn | grep -q ":9000"; then
  echo "[RESTART] MinIO already on :9000"
else
  echo "[RESTART] Starting MinIO via Apptainer on :9000 ..."
...
# Start backend
echo "[RESTART] Starting backend (uvicorn :8000) ..."
...
if command -v celery >/dev/null 2>&1; then
  ...
  echo "[RESTART] Detected GPUs: ${GPU_COUNT}"
  ...
  echo "[RESTART] ERROR: celery not found in PATH. Install with: pip install celery"
fi

# Start frontend
echo "[RESTART] Starting frontend (Vite :56052) ..."
if ! ss -ltn | grep -q ":56052"; then
  ...
  echo "[RESTART] Vite already on :56052"
fi
...
echo -n "[RESTART] Backend /health: "; curl -sI http://127.0.0.1:8000/health | head -n1 || true
echo -n "[RESTART] Proxy /api/health: "; curl -sI http://127.0.0.1:56052/api/health | head -n1 || true
echo -n "[RESTART] Proxy /api/jobs:   "; curl -sI http://127.0.0.1:56052/api/jobs/ | head -n1 || true
echo -n "[RESTART] MinIO live:        "; curl -sI http://127.0.0.1:9000/minio/health/live | head -n1 || true

echo "[RESTART] Ready. Open UI: http://localhost:56052/"
echo "[RESTART] Logs:"
