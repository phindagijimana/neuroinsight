...
```
backend/api/upload.py                    ✅ Fixed in v1.3.10
desktop_alone/backend/api/upload.py      ❌ Still had old code (until now!)
```

---

## 🔍 **Evidence from Windows Testing Logs**
...
- ❌ File: `sub-268_T1w.nii` (44.5 MB - valid size!)
- ❌ Result: `400 Bad Request`
- ❌ Error details: **NONE** (old code had no error logging)
...
if file.filename.endswith((".nii", ".nii.gz")):
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        if any(z > 5.0 for z in zooms) or any(z < 0.2 for z in zooms):
            raise HTTPException(...)  # ❌ Too strict!
...
|-------|--------------|---------------|
| File extension | ✅ .nii, .nii.gz, .dcm | ✅ Same |
| File size | ✅ 0 < size < 1GB | ✅ Same |
| **Filename contains "T1"** | ❌ Not checked | ✅ Required |
| **NIfTI parsing** | ✅ Full parse | ❌ Removed |
| **Voxel spacing** | ✅ 0.2-5.0mm strict | ❌ Removed |
| **3D/4D shape check** | ✅ Yes | ❌ Removed |
| **Dimension minimum** | ✅ 32x32x32 | ❌ Removed |
| **Zeros/NaN check** | ✅ Yes | ❌ Removed |
| **Header parsing** | ✅ T1 markers | ❌ Removed |
| **DICOM metadata** | ✅ SeriesDescription | ❌ Removed |
...
### **3. Performance**
- OLD: Parses 44MB file, reads entire data array → 10+ seconds
- NEW: Checks filename → instant
...
- ✅ Filename: `sub-268_T1w.nii` contains "T1" 
- ✅ Extension: `.nii` is valid
- ✅ Size: 44.5 MB is within 1GB limit
- ✅ No voxel spacing check anymore!
...
```
backend/api/upload.py                    ← Web app (fixed in v1.3.10)
desktop_alone/backend/api/upload.py      ← Desktop app (fixed in v1.3.11)
```
...
**Status:** ✅ Fixed in code, ready to build and test  
