...
if echo "$METRICS" | grep -q "Hippocampus"; then
    ...
    echo "   WARNING: No metrics in job data"
fi
echo ""
...
if echo "$SLICE_RESPONSE" | grep -q "image/png"; then
    ...
    echo "   WARNING: Could not verify image format"
fi
echo ""
...
if [ -d "$VIZ_DIR" ]; then
    ...
    echo "   ERROR: Visualization directory not found: $VIZ_DIR"
fi
echo ""
...
echo "Test Summary"
echo "=========================================="
echo "SUCCESS: Job exists and is completed"
echo "SUCCESS: Metrics available in job data"
echo "SUCCESS: $SLICE_COUNT slice visualizations available via API"
echo ""
